...
class RecordingHandler : public xmlparser::v1::SaxHandler {
 public:
  ...
  void start_element(
    ...
    RecordedEvent event{"start_element", name.uri, name.local_name,
                        name.qualified_name, "", {}};
    for (const auto& attribute : attributes) {
      event.attributes.push_back({attribute.name.uri, attribute.name.local_name,
  ...
  void end_element(const xmlparser::v1::QualifiedName& name) override {
    events.push_back({"end_element", name.uri, name.local_name,
                      name.qualified_name, "", {}});
  }
